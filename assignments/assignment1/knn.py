...
class KNN:
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            nearest_k_ids = np.argsort(dists[i,:])[:self.k]
            pred[i] = np.mean(self.train_y[nearest_k_ids]) > 0.5
        return pred
    ...
    def predict_labels_multiclass(self, dists):
        ...
        for i in range(num_test):
            ...
            # nearest training samples
            nearest_k_ids = np.argsort(dists[i,:])[:self.k]
            counts = [(self.train_y[nearest_k_ids] == number).sum() for number in range(10)]
